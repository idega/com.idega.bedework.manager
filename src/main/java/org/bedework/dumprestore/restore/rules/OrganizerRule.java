...
public class OrganizerRule extends EntityRule {
  ...
  public void end(final String ns, final String name) throws Exception {
    OrganizerEntity entity = (OrganizerEntity)pop();

    if (!thisOrAfterVersion(3, 5)) {
      // Store it in a table to stuff into the event later

      globals.organizersTbl.put(entity);
    } else {
      EventInfo ei = (EventInfo)getTop(EventInfo.class, name);

      BwEvent e = ei.getEvent();

      e.setOrganizer(entity.organizer);
    }
  }
}
