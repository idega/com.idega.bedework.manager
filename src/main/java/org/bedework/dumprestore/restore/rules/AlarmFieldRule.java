...
public class AlarmFieldRule extends EntityFieldRule {
  ...
  private boolean pre3p5(final BwAlarm ent, final String name) throws Throwable {
    ...
    if (name.equals("trigger-start")) {
      ent.setTriggerStart(booleanFld());
      return true;
    }

    if (name.equals("trigger-time")) {
      ent.setTriggerTime(stringFld());
      return true;
    }

    if (name.equals("repeat-count")) {
      ent.setRepeatCount(intFld());
      return true;
    }

    if (name.equals("previous-trigger")) {
      ent.setPreviousTrigger(stringFld());
      return true;
    }
